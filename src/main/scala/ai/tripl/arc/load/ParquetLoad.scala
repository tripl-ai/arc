...
class ParquetLoad extends PipelineStagePlugin with JupyterCompleter {
  ...
  val documentationURI = new java.net.URI(s"${baseURI}/load/#parquetload")
...
object ParquetLoadStage {
  ...
  def execute(stage: ParquetLoadStage)(implicit spark: SparkSession, logger: ai.tripl.arc.util.log.logger.Logger, arcContext: ARCContext): Option[DataFrame] = {
    ...
    // ParquetLoad cannot handle a column of NullType
    ...
    val nonNullDF = if (!nulls.isEmpty) {
      ...
      if (arcContext.dropUnsupported) {
        ...
        throw new Exception(s"""inputView '${stage.inputView}' contains types ${new com.fasterxml.jackson.databind.ObjectMapper().writeValueAsString(dropMap)} which are unsupported by ParquetLoad and 'dropUnsupported' is set to false.""")
      }
    } else {
      df
    }
        ...
        stage.partitionBy match {
          case Nil => {
            stage.numPartitions match {
              case Some(n) => nonNullDF.repartition(n).write.mode(stage.saveMode).format("parquet").save(stage.outputURI.toString)
              case None => nonNullDF.write.mode(stage.saveMode).format("parquet").save(stage.outputURI.toString)
            }
          }
          case partitionBy => {
            ...
            stage.numPartitions match {
              case Some(n) => nonNullDF.repartition(n, partitionCols:_*).write.partitionBy(partitionBy:_*).mode(stage.saveMode).format("parquet").save(stage.outputURI.toString)
              case None => nonNullDF.repartition(partitionCols:_*).write.partitionBy(partitionBy:_*).mode(stage.saveMode).format("parquet").save(stage.outputURI.toString)
            }
          }
        }
      }
