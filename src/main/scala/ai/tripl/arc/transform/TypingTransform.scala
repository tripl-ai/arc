...
class TypingTransform extends PipelineStagePlugin {
  ...
  def instantiate(index: Int, config: com.typesafe.config.Config)(implicit spark: SparkSession, logger: ai.tripl.arc.util.log.logger.Logger, arcContext: ARCContext): Either[List[ai.tripl.arc.config.Error.StageError], PipelineStage] = {
    ...
    val extractColumns = if(!c.hasPath("schemaView")) getValue[String]("inputURI") |> parseURI("inputURI") _ |> getExtractColumns("inputURI", authentication) _ else Right(List.empty)
