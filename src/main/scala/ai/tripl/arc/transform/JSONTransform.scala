...
class JSONTransform extends PipelineStagePlugin with JupyterCompleter {
  ...
  val documentationURI = new java.net.URI(s"${baseURI}/transform/#jsontransform")
  ...
  def instantiate(index: Int, config: com.typesafe.config.Config)(implicit spark: SparkSession, logger: ai.tripl.arc.util.log.logger.Logger, arcContext: ARCContext): Either[List[ai.tripl.arc.config.Error.StageError], PipelineStage] = {
    ...
    (id, name, description, inputView, outputView, persist, numPartitions, partitionBy, invalidKeys) match {
      case (Right(id), Right(name), Right(description), Right(inputView), Right(outputView), Right(persist), Right(numPartitions), Right(partitionBy), Right(invalidKeys)) =>
