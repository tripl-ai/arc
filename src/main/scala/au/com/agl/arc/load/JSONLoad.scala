    ...
    try {
      if (df.isStreaming) {
        load.partitionBy match {
          case Nil => df.writeStream.format("json").option("path", load.outputURI.toString).start
          case partitionBy => {
            val partitionCols = partitionBy.map(col => df(col))
            df.writeStream.partitionBy(partitionBy:_*).format("json").option("path", load.outputURI.toString).start
          }
        }
      } else {
        load.partitionBy match {
